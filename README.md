...
* **진화 및 한계점**:
  * 🔺 **테두리(Border) 내부 침범**: 꼬리 2면에도 핫핑크 테두리가 연결되지만, **테두리 선이 본체 내부 경계를 미세하게 침범(🔺)**함.
...
| :--- | :---: | :---: | :---: | :---: |
| 💖 **1. 네온 핫핑크 테두리 (3px)** | ❌ **꼬리 누락** | 🔺 **내부 침범** | ⭕ Drop-shadow 해결 | ⭕ **완벽 일체형** |
| 🧊 **2. 반투명 글래스 (Glass)** | ❌ 트릭 붕괴 | ❌ **본체 안 꼬리 사각형 투과** | ⭕ **외부 돌출 해결** | ⭕ **완벽 단일 면적** |
...
```html
<!-- 독립 SVG 꼬리 엘리먼트 결합 -->
<svg class="absolute -left-3 top-[11px] w-3.5 h-4">
...
```css
/* 4개 모서리 라운드 곡률(12px)과 꼬리 정중앙(Y=20px) 완벽 일체형 절삭 */
.single-clip-wrapper {
  clip-path: polygon(
